...
def include_memory_link(np_matrices,memory):
    """Extends duration of a link to the defined memory. For example suppose that at time t=3 the is a link between nodes i & j.
...
def make_loop(np_matrices):
    '''Takes a series of adjacency matrix snapshots and doubles the length looping around the same set of snapshots twice'''
...
def temporal_network_clust_plot(np_matrices,memory):
    """Calculates the number of triangles and triples based on searching the network using adjacency matrices and plots the results"""
    snapshots = len(np_matrices)
    triples = [0]*(snapshots)
...
#     np_matrices_mem = include_memory(np_matrices,memory)
...
#             for i in range(nrows):
#                 for j in range(ncols):
#                     if (access[k][i,j] == 1) and (access[k-1][i,j]==0):
#                         times[i][j] = k+1
...
#         else:
#             paths_active.append((np.sum(times))/(start_nodes*(active_nodes-1)))
...
def temporal_path_length(np_matrices,memory):
    '''Implementation of algorithm outlined in the paper. Calculate the shortest temporal path length utilising unfolding accessibility'''
...
def temporal_network_analysis(adjacency_matrices,memory,max_window):
    """Pipeline of the clustering functions."""
    np_matrices={}
    to_numpy_matrix(adjacency_matrices,max_window,np_matrices)
    nrows = ncols = len(np_matrices[0])
    
    
    np_matrices={}
    to_numpy_matrix(adjacency_matrices,max_window,np_matrices)
    cluster = temporal_network_clustering(np_matrices,memory)
    
    
    np_matrices={}
    to_numpy_matrix(adjacency_matrices,max_window,np_matrices)
    temporal_network_clust_plot(np_matrices,memory)
    print('*****The network clustering is:',cluster)


#Hartmut Lentz Class defintion
